...
def read_frames(video_path, train_dataset, validation_dataset, test_dataset, image_size, config):
    # Determine the label based on the directory name
    if "original" in video_path.lower():
        label = 0.  # Real
    else:
        label = 1.  # Fake (any other dataset)
    ...
    if VALIDATION_DIR in video_path or TEST_DIR in video_path:
...
if __name__ == "__main__":
    ...
    #READ DATASET
    folders = ["original"] + [folder.lower() for folder in os.listdir(TRAINING_DIR) if folder.lower() != "original"]
    
    sets = [TRAINING_DIR, VALIDATION_DIR, TEST_DIR]
        ...
        for folder in folders:
            ...
            if os.path.isdir(subfolder):
                for index, video_folder_name in enumerate(os.listdir(subfolder)):
                    if opt.max_videos != -1 and index == opt.max_videos:
                        break
    ...
    print("Real:", "original")
    print("Fake:", [folder for folder in folders if folder != "original"])
    ...
    with Pool(processes=opt.workers) as p:
        with tqdm(total=len(paths)) as pbar:
            for v in p.imap_unordered(partial(read_frames, train_dataset=train_dataset, validation_dataset=validation_dataset, test_dataset=test_dataset, image_size=image_size, config=config), paths):
                pbar.update()
    ...
    print("Train images:", len(train_dataset), "Validation images:", len(validation_dataset), "Test images:", len(test_dataset))
    ...
    test_counters = collections.Counter(image[1] for image in test_dataset)
    print(test_counters)
    ...
    test_labels = np.asarray([row[1] for row in test_dataset])
    ...
    test_dataset = DeepFakesDataset(np.asarray([row[0] for row in test_dataset]), test_labels, image_size, mode='test')
    test_dl = torch.utils.data.DataLoader(test_dataset, batch_size=config['training']['bs'], shuffle=False, sampler=None,
                                    batch_sampler=None, num_workers=opt.workers, collate_fn=None,
                                    pin_memory=False, drop_last=False, timeout=0,
                                    worker_init_fn=None, prefetch_factor=2,
                                    persistent_workers=False)
    ...
    for t in range(starting_epoch, opt.num_epochs + 1):
        ...
        for index, (images, labels) in enumerate(dl):
            images = np.transpose(images, (0, 3, 1, 2)).cuda()
        ...
        for index, (val_images, val_labels) in enumerate(val_dl):
            ...
            with autocast():
                val_pred = model(val_images).cuda()
        ...
        torch.save(model.state_dict(), os.path.join(MODELS_PATH, "efficientnet_checkpoint" + str(t) + "_all"))
    ...
    with torch.no_grad():
        for test_images, test_labels in test_dl:
            test_images = np.transpose(test_images, (0, 3, 1, 2)).cuda()
            test_labels = test_labels.unsqueeze(1).cuda()
            with autocast():
            ...
            test_counter += 1
            all_test_labels.extend(test_labels.cpu().numpy().flatten())
    ...
    print(f"Test Real (0s): {test_negative}/{np.count_nonzero(np.array(all_test_labels) == 0)}")
    print(f"Test Fake (1s): {test_positive}/{np.count_nonzero(np.array(all_test_labels) == 1)}")

    # Save the final model
    torch.save(model.state_dict(), os.path.join(MODELS_PATH, "efficientnet_final_model_all"))
